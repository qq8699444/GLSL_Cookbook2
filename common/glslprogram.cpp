...
{
  GLint loc = getUniformLocation(name); checkGLError("glBindVertexArray");
  glUniform3f(loc,x,y,z); checkGLError("glBindVertexArray");
}
...
{
  GLint loc = getUniformLocation(name); checkGLError("getUniformLocation");
  glUniform4f(loc,v.x,v.y,v.z,v.w); checkGLError("glBindVertexArray");
}

void GLSLProgram::setUniform( const char *name, const vec2 & v)
{
  GLint loc = getUniformLocation(name); checkGLError("getUniformLocation");
  glUniform2f(loc,v.x,v.y); checkGLError("glBindVertexArray");
}

void GLSLProgram::setUniform( const char *name, const mat4 & m)
{
  GLint loc = getUniformLocation(name); checkGLError("getUniformLocation");
  glUniformMatrix4fv(loc, 1, GL_FALSE, &m[0][0]); checkGLError("glBindVertexArray");
}

void GLSLProgram::setUniform( const char *name, const mat3 & m)
{
  GLint loc = getUniformLocation(name); checkGLError("getUniformLocation");
  glUniformMatrix3fv(loc, 1, GL_FALSE, &m[0][0]); checkGLError("glBindVertexArray");
}

void GLSLProgram::setUniform( const char *name, float val )
{
  GLint loc = getUniformLocation(name); checkGLError("getUniformLocation");
  glUniform1f(loc, val); checkGLError("glUniform1f");
}
