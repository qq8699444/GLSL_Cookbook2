        ...
        rings(nrings), sides(nsides)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, handle[0]); checkGLError("glBindVertexArray");
    glVertexAttribPointer( (GLuint)0, 3, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );

    glEnableVertexAttribArray(1);  // Vertex normal
    glBindBuffer(GL_ARRAY_BUFFER, handle[1]); checkGLError("glBindVertexArray");
    glVertexAttribPointer( (GLuint)1, 3, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );

    glBindBuffer(GL_ARRAY_BUFFER, handle[2]); checkGLError("glBindVertexArray");
    glEnableVertexAttribArray(2);  checkGLError("glEnableVertexAttribArray 2"); // Texture coords
    glVertexAttribPointer( (GLuint)2, 2, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, handle[3]); checkGLError("glBindVertexArray");

    glBindVertexArray(0); checkGLError("glBindVertexArray");
}

void VBOTorus::render() const {
    glBindVertexArray(vaoHandle); checkGLError("glBindVertexArray");
    glDrawElements(GL_TRIANGLES, 6 * faces, GL_UNSIGNED_INT, ((GLubyte *)NULL + (0))); checkGLError("glDrawElements");
}
